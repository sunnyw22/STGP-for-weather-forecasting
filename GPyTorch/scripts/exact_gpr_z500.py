...
from GPyTorch.GPRs.utils import (
    splitting_data,
    extreme_points_rel_err,
    extreme_points_rmse,
)
...
matern_spatial_1 = gpytorch.kernels.MaternKernel(nu=1.5, active_dims=[0, 1, 2]).to(
    device
)
matern_spatial_1.lengthscale = torch.tensor(spatial_lengths[0]).to(device)
matern_spatial_2 = gpytorch.kernels.MaternKernel(nu=1.5, active_dims=[0, 1, 2]).to(
    device
)
matern_spatial_2.lengthscale = torch.tensor(spatial_lengths[1]).to(device)
matern_spatial_3 = gpytorch.kernels.MaternKernel(nu=1.5, active_dims=[0, 1, 2]).to(
    device
)
...
def train_gp(
    ...
    # Random sampling: Here we select n_frac points (e.g., 10,000)
    n_frac = 10000
    ...
    for epoch in tqdm(range(epochs), desc="Training Progress", unit="epoch"):
        ...
        spatial_kernel_mod = model.covar_module.base_kernel.kernels[0]
        temporal_kernel_mod = model.covar_module.base_kernel.kernels[1]
        hyperparams_track.append(
            {
                ...
                "spatial_1_lengthscale": spatial_kernel_mod.kernels[
                    0
                ].lengthscale.item(),
                "spatial_2_lengthscale": spatial_kernel_mod.kernels[
                    1
                ].lengthscale.item(),
                "spatial_3_lengthscale": spatial_kernel_mod.kernels[
                    2
                ].lengthscale.item(),
                "time_matern_lengthscale": temporal_kernel_mod.kernels[
                    0
                ].lengthscale.item(),
                "time_lengthscale_1": temporal_kernel_mod.kernels[1].lengthscale.item(),
                "time_period_1": temporal_kernel_mod.kernels[1].period_length.item(),
                "time_lengthscale_2": temporal_kernel_mod.kernels[2].lengthscale.item(),
                "time_period_2": temporal_kernel_mod.kernels[2].period_length.item(),
                "time_lengthscale_3": temporal_kernel_mod.kernels[3].lengthscale.item(),
                "time_period_3": temporal_kernel_mod.kernels[3].period_length.item(),
            }
        )
...
def predictions(
    ...
    X_test, Y_test, lat, lon = create_data(
        data_train,
        data_test,
        lead_time,
        space_subsample=space_subsample,
    ...
    with torch.no_grad():
        ...
        Y_pred_actual = (
            Y_pred.detach().cpu().numpy() * data_std.values + data_mean.values
        ).reshape(-1, nlat, nlon)
        Y_test_actual = (
            Y_test.detach().cpu().numpy() * data_std.values + data_mean.values
        ).reshape(-1, nlat, nlon)
...
def main():
    ...
    z500 = xr.open_mfdataset(
        "data/5.625deg/geopotential_500/*.nc", combine="by_coords"
    ).sel(time=slice("2000", "2018"))
    ...
    for i in range(num_lat_bounds):
        for j in range(num_lon_bounds):
            ...
            z500_train = z500.sel(time=slice("2017", "2017")).isel(
                lat=lat_slice, lon=lon_slice
            )["z"]
            z500_test = z500.sel(time=slice("2018", "2018.1.7")).isel(
                lat=lat_slice, lon=lon_slice
            )["z"]
            ...
            model, likelihood, hyperparams_track, X_train, Y_train = train_gp(
                lead_time,
                data_train,
                data_test,
                space_subsample,
            ...
            target_folder = (
                f"plots/5.625deg/z500/Train (2017), test (2018_1_7)/ExactGP_full/"
            ...
            full_relative_error_grid[
                lat_bound[i] : lat_bound[i + 1], lon_bound[j] : lon_bound[j + 1]
            ] = relative_error_mean
            full_rmse_error_grid[
                lat_bound[i] : lat_bound[i + 1], lon_bound[j] : lon_bound[j + 1]
            ] = rmse_error_mean
    ...
    target_folder_final = (
        "plots/5.625deg/z500/Train (2017), test (2018_1_7)/ExactGP_full/final"
    )
